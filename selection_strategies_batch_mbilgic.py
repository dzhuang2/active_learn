...
class Pipe(object):
    def __init__(self, strategies, counts):
        self.strategies = strategies
        self.counts = counts
        self.current_count = 0
        self.current_strategy = None
    
    def choices(self, model, X, pool, k, discovered_class0_feats, discovered_class1_feats, top_k=10):
        if self.current_count <= 0:
            self.current_count = self.counts.pop(0)
            self.current_strategy = self.strategies.pop(0)
        if isinstance(self.current_strategy, UNCSampling):
            chosen = self.current_strategy.choices(model, X, pool, k)
        else:
            chosen = self.current_strategy.choices(model, X, pool, k, discovered_class0_feats, discovered_class1_feats, top_k)
        self.current_count -= 1
        return chosen

class UNCPreferNoRationale(object):
    '''
    This class picks uncertain instances.
    It prefers uncertain instances that do not have any annotated feature.
    '''
    ...
    def choices(self, model, X, pool, k, discovered_class0_feats, discovered_class1_feats, top_k=10):
        ...
        for unc in uncerts[:top_k]:
            ...
            if (len(x_class0_feats) == 0) and (len(x_class1_feats) == 0):
                has_rationale.append(False)
            else:
                has_rationale.append(True)
            
        
        index = 0
        chosen = set()
        while len(chosen) < k and index < top_k:
            if not has_rationale[index]:
                chosen.add(uncerts[index])
            index += 1
        
        
        if len(chosen) < k: # Not enough found
            index = 0        
            while len(chosen) < k and index < top_k:            
                chosen.add(uncerts[index])
                index += 1         
...
class UNCPreferRationale(object):
    '''
    This class picks uncertain instances.
    It prefers uncertain instances that have at least one annotated feature.
    '''
