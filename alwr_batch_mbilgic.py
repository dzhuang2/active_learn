...
def run_trials(model_type, num_trials, dataset, tfidf, selection_strategy, metric, C, alpha, \
                bootstrap_size, balance, budget, step_size, topk, w_o, w_r, seed=0, lr_C=1, svm_C=1, Debug=False):
    ...
    for i in range(num_trials):
        ...
        result[i] = learn(model_type, X_pool, y_pool, X_test, y_test, training_set, pool_set, fe, \
                          selection_strategy, budget, step_size, topk, w_o, w_r, trial_seed, lr_C, svm_C, Debug)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-model_type', choices=['lrl2', 'lrl1', 'mnb', 'svm_linear'], default='lrl2', help='Type of classifier to be used')
    ...
    save_result(average_results(result), filename='_'.join([args.dataset[0], 'tfidf{}'.format(args.tfidf), args.file_tag, args.model_type, args.strategy, args.metric, 'lr_C={:5.3f}'.format(args.lr_C), 'SVM_C={:5.3f}'.format(args.svm_C), 'w_r={:2.3f}'.format(args.w_r), 'w_o={:2.3f}'.format(args.w_o), 'averaged', 'batch-result.txt']))
